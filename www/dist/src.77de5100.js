...
function start(websocketServerLocation) {
  ...
  ws.onclose = function () {
    setTimeout(function () {
      return start(websocketServerLocation);
    }, 5000);
  };
}
...
function updateParticipants(participants, participant) {
  var idx = participants.findIndex(function (_a) {
    var penUrl = _a.penUrl;
    return penUrl === participant.penUrl;
  });

  if (idx === -1) {
    return participants.concat([participant]);
  } else {
    var copy = participants.slice(0);
...
document.querySelector('.registration-form').addEventListener('submit', formSubmit);

function formSubmit(e) {
  e.preventDefault();
  var errMsgElt = document.querySelector('.registration-error');
  var okMsgElt = document.querySelector('.registration-ok');
  var form = e.target;
  fetch('/register', {
    method: 'POST',
    headers: {
    ...
    if (json.error) {
      ...
      errMsgElt.innerHTML = json.error; // document.querySelector('.registration-ok').style.display = 'initial';
    } else {
      form.reset();
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "43991" + '/');
